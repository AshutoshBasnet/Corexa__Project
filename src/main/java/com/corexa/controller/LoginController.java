...
import com.corexa.service.LoginService;
import com.corexa.util.CookieUtil;
import com.corexa.util.RedirectionUtil;
import com.corexa.util.SessionUtil;
...
public class LoginController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        String validationMessage = validateLoginForm(username, password);
    ...
    private void handleLoginFailure(HttpServletRequest req, HttpServletResponse resp)
            throws ServletException, IOException {
        String errorMessage = loginService.getLastFailureReason();
    ...
    private String validateLoginForm(String username, String password) {
        boolean isUsernameEmpty = ValidationUtil.isNullOrEmpty(username);
        boolean isPasswordEmpty = ValidationUtil.isNullOrEmpty(password);

        if (isUsernameEmpty && isPasswordEmpty) {
            return "Username and password are required to login.";
        }
        if (isUsernameEmpty) {
            return "Username is required to login.";
        }
        if (isPasswordEmpty) {
