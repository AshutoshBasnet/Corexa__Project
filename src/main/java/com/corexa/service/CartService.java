...
import com.corexa.model.CartModel;
import com.corexa.model.CartItemModel;
...
public class CartService {
    ...
    public CartModel getCartByUserId(int userId) {
        String cartQuery = "SELECT * FROM corexa.cart WHERE user_id = ?";
        String itemsQuery = "SELECT ci.*, p.* FROM corexa.cartitem ci JOIN corexa.product p ON ci.product_id = p.product_id WHERE ci.cart_id = ?";
    ...
    public boolean clearCart(int userId) {
        try {
            ...
            String deleteItems = "DELETE FROM corexa.cartitem WHERE cart_id = ?";
            ...
            // Reset cart totals
            String resetCart = "UPDATE corexa.cart SET total_items = 0, total_price = 0 WHERE cart_id = ?";
    ...
    private int getOrCreateCartId(int userId) throws SQLException {
        int cartId = getCartId(userId);
        if (cartId == 0) {
            String query = "INSERT INTO corexa.cart (user_id, total_items, total_price, createdAt) VALUES (?, 0, 0, ?)";
    ...
    private int getCartId(int userId) throws SQLException {
        String query = "SELECT cart_id FROM corexa.cart WHERE user_id = ?";
    ...
    private CartItemModel getCartItem(int cartId, int productId) throws SQLException {
        String query = "SELECT ci.*, p.* FROM corexa.cartitem ci JOIN corexa.product p ON ci.product_id = p.product_id " +
    ...
    private boolean insertCartItem(int cartId, int productId, int quantity) throws SQLException {
        String query = "INSERT INTO corexa.cartitem (cart_id, product_id, productQuantity) VALUES (?, ?, ?)";
        try (PreparedStatement stmt = dbConn.prepareStatement(query)) {
            stmt.setInt(1, cartId);
            stmt.setInt(2, productId);
    ...
    private boolean updateCartItemQuantity(int cartId, int productId, int newQuantity) throws SQLException {
        String query = "UPDATE corexa.cartitem SET productQuantity = ? WHERE cart_id = ? AND product_id = ?";
        try (PreparedStatement stmt = dbConn.prepareStatement(query)) {
    ...
    private boolean deleteCartItem(int cartId, int productId) throws SQLException {
        String query = "DELETE FROM corexa.cartitem WHERE cart_id = ? AND product_id = ?";
        try (PreparedStatement stmt = dbConn.prepareStatement(query)) {
            stmt.setInt(1, cartId);
            stmt.setInt(2, productId);
            return stmt.executeUpdate() > 0;
    ...
    private void updateCartTotals(int cartId) throws SQLException {
        String query = "UPDATE corexa.cart c SET " +
                      "total_items = (SELECT COALESCE(SUM(productQuantity), 0) FROM corexa.cartitem WHERE cart_id = ?), " +
                      "total_price = (SELECT COALESCE(SUM(p.price * ci.productQuantity), 0) " +
                      "FROM corexa.cartitem ci JOIN corexa.product p ON ci.product_id = p.product_id WHERE ci.cart_id = ?) " +
                      "WHERE c.cart_id = ?";
